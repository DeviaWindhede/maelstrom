...
private:
	void RemoveComponentInternal(const Entity& aEntity, EntitySignatureCollection& aCollection)
	{
		...
		int indexOfRemovedEntity = aCollection.GetSignatureIndex(aEntity, T::componentId);
		...
		aCollection.SetSignatureIndex(lastEntity, T::componentId, indexOfRemovedEntity);
